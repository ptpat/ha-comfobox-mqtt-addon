...
if [ -f "$CFG" ]; then
  ...
  # *** ONLY NEW: force BacnetMasterId to 11 (based on your log OnIam DeviceId=11) ***
  # This targets the specific setting block and replaces the <value>...</value>.
  sed -i -E 's|(<setting name="BacnetMasterId"[^>]*>[[:space:]]*<value>)[^<]*(</value>)|\111\2|g' "$CFG" || true
fi
